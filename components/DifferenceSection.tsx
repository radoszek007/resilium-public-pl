  ...
  return (
    <section className="bg-resilium-dark py-24 text-white sm:py-32">
      <div className="section-shell">
        ...
        <div className="mt-14 grid overflow-hidden rounded-[30px] border border-[#E5EAF0] bg-resilium-card shadow-soft lg:grid-cols-2">
          <div className="border-b border-[#E5EAF0] p-7 lg:border-b-0 lg:border-r">
            <h3 className="text-2xl font-black text-resilium-ink">Typowy kurs rozwoju osobistego</h3>
            <ul className="mt-8 space-y-4">
              {common.map((item) => (
                <li key={item} className="flex gap-3 text-resilium-muted">
                  <span className="mt-2 h-2 w-2 shrink-0 rounded-full bg-resilium-muted/40" />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
          </div>
          <div className="bg-resilium-slate p-7">
            <h3 className="text-2xl font-black text-resilium-orange">Resilium Public</h3>
            <ul className="mt-8 space-y-4">
              {resilium.map((item) => (
                <li key={item} className="flex gap-3 text-resilium-ink">
